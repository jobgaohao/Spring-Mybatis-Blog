var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderGroup: function(target, groupIndex, group, frozen){
		...
		if (!frozen){
			table.push('<span class="datagrid-group-title-expander datagrid-group-title-collapse">');
		...
		//添加表头开始
		var colName=[];
		table.push('<tr class=\"datagrid-header-row\" style=\"text-align: center;background-color: #DDD;\">');
		for(i=0;i<fields.length;i++)
		{
			if(fields[i] == 'stockinCode'){
				continue;
			}else{
				var col = $(target).datagrid( "getColumnOption" , fields[i] );
				colName.push(col.title);//把TITLEPUSH到数组里去
				table.push('<td><span>'+col.title +'</span></td>');
			}
		}
		table.push('</tr>');
		//添加表头结束
	...
	//点击标题展开明细
	bindEventsTitil: function(target){
		var state = $.data(target, 'datagrid');
		var dc = state.dc;	
		...
		body.find("th").each(function(){			
			var _this=$(this);
			if(!_this.hasClass("qa_btn")){
				_this.unbind("click").bind("click",function(e){
					var tt = $(e.target);
					var expander = tt.closest('span.datagrid-group-title-expander');
					if (expander.length){
						var gindex = expander.closest('div.datagrid-group').attr('group-index');
						if (expander.hasClass('datagrid-group-title-collapse')){
		...
		function initCss(){
			if (!$('#datagrid-group-style').length){
				$('head').append(
					...
					'.datagrid-group{font-weight:normal;color: #0F4A80;border-top:2px solid #95B8E7;background-color: #E0ECFF;background: linear-gradient(to bottom,#EFF5FF 0,#E0ECFF 100%);background-repeat: repeat-x;}.datagrid-group table{width:100%;}' +
					'</style>'
				);
			}
		}
...
$.extend($.fn.datagrid.methods, {
    expandGroup:function(jq, groupIndex){
        return jq.each(function(){
            ...
            var expander = group.find('span.datagrid-group-title-expander');
            if (expander.hasClass('datagrid-group-title-expand')){
                expander.removeClass('datagrid-group-title-expand').addClass('datagrid-group-title-collapse');
                group.next('table').show();
      }
    ...
    collapseGroup:function(jq, groupIndex){
        return jq.each(function(){
            ...
            var expander = group.find('span.datagrid-group-title-expander');
            if (expander.hasClass('datagrid-group-title-collapse')){
                expander.removeClass('datagrid-group-title-collapse').addClass('datagrid-group-title-expand');
                 group.next('table').hide();
      }
            $(this).datagrid('fixRowHeight');
        });
